# https://github.com/Serock3/Master_thesis_QEC_simulation/blob/bfc808d5e73b7e0bc77f7a36bdc373f7d27a4c8f/decay_comparison.py
""" 
...
def project_dm_to_logical_subspace_V1(rho):
    ...
    P_L += logical[0] @ rho @ logical[0]
    P_L += logical[1] @ rho @ logical[1]
    rho_L = np.zeros((2, 2), dtype=complex)
    rho_L[0, 0] = (logical[0] @ rho @ logical[0])/P_L
    rho_L[0, 1] = (logical[0] @ rho @ logical[1])/P_L
    rho_L[1, 0] = (logical[1] @ rho @ logical[0])/P_L
    rho_L[1, 1] = (logical[1] @ rho @ logical[1])/P_L
    return rho_L
...
def project_dm_to_logical_subspace_V2(rho):
    ...
    rho_L += logical_pauli_matrices[0] * \
            np.trace(rho@logical_pauli_matrices[0])/(2*P_L)
    rho_L += logical_pauli_matrices[1] * \
            np.trace(rho@logical_pauli_matrices[1])/(2*P_L)
    rho_L += logical_pauli_matrices[2] * \
            np.trace(rho@logical_pauli_matrices[2])/(2*P_L)
    rho_L += logical_pauli_matrices[3] * \
            np.trace(rho@logical_pauli_matrices[3])/(2*P_L)
    return rho_L
...
def project_dm_to_logical_subspace_V3(rho):
    ...
    rho_L = np.zeros((2, 2), dtype=complex)
    rho_L += pauli_matrices[0] * \
            np.trace(rho@logical_pauli_matrices[0])/(2*P_L)
    rho_L += pauli_matrices[1] * \
            np.trace(rho@logical_pauli_matrices[1])/(2*P_L)
    rho_L += pauli_matrices[2] * \
            np.trace(rho@logical_pauli_matrices[2])/(2*P_L)
    rho_L += pauli_matrices[3] * \
            np.trace(rho@logical_pauli_matrices[3])/(2*P_L)
    return rho_L
