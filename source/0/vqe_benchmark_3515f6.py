# https://github.com/rsathyak/qml_masters_thesis/blob/02143908c3f2a5125b189d6af0dbff54436622c1/experiments/qmetrics/examples/qiskit/vqe/vqe_benchmark.py
"""
...
def VQEEnergy(n_spin_orbs, na, nb, circuit_id=0, method=1):
    ...
    # add to coefficients of diagonal terms
    diag_coeff = abs(qubit_op[1].coeffs[0])
    ...
    qc_list[0].name = qubit_op[1].primitive.to_list()[0][0] + " " + \
        str(np.real(diag_coeff))
...
def ClusterOperatorCircuit(pauli_op, excitationIndex):
    ...
    qc_op = PauliTrotterEvolution(
        trotter_mode=Suzuki(order=1, reps=1)).convert(exp_ip)
...
def readPauliExcitation(norb, circuit_id=0):
    ...
    filename = os.path.join(os.path.dirname(__file__),
                            f'./ansatzes/{norb}_qubit_{circuit_id}.txt')
...
def ReadHamiltonian(nqubit):
    ...
    # load pre-computed data
    filename = os.path.join(os.path.dirname(__file__),
                            f'./Hamiltonians/{nqubit}_qubit.txt')
    with open(filename) as f:
        data = f.read()
...
def run(min_qubits=4, max_qubits=10, max_circuits=3, num_shots=4092, method=1,
    ...
    def execution_handler(qc, result, num_qubits, type, num_shots):
        ...
        if len(qc.name.split()) == 2:
            metrics.store_metric(num_qubits, qc.name.split()[
                                 0], 'fidelity', fidelity)
        else:
            metrics.store_metric(num_qubits, qc.name.split()[
                                 2], 'fidelity', fidelity)
    ...
    for input_size in range(min_qubits, max_qubits + 1, 2):
        ...
        for qc in qc_list:
            ...
            metrics.store_metric(input_size, circuit_id,
    ...
    metrics.plot_metrics(
        f"Benchmark Results - VQE Simulation ({method}) - Qiskit")
