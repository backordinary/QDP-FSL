# https://github.com/jason-jk-kang/QIS-qchem/blob/6f296a4272d842a136892e33f8edd77d2051fc60/ProjectQ/openqasm_test.py
...
def test_openqasm_allocate_deallocate(qubit_id_redux):
    ...
    qasm = qc_list[1].qasm()
    assert re.search(r'qreg\s+q{}\[1\]'.format(1), qasm)
    assert re.search(r'creg\s+c{}\[1\]'.format(1), qasm)
    assert re.search(r'qreg\s+q{}\[1\]'.format(2), qasm)
    assert re.search(r'creg\s+c{}\[1\]'.format(2), qasm)
    assert re.search(r'qreg\s+q{}\[1\]'.format(3), qasm)
    assert re.search(r'creg\s+c{}\[1\]'.format(3), qasm)
    assert re.search(r'qreg\s+q{}\[1\]'.format(4), qasm)
    assert re.search(r'creg\s+c{}\[1\]'.format(4), qasm)
    assert re.search(r'qreg\s+q{}\[1\]'.format(5), qasm)
    assert re.search(r'creg\s+c{}\[1\]'.format(5), qasm)
