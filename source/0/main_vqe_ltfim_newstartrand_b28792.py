# https://github.com/AntMele/Avoiding-Barren-Plateaus-via-transferability-of-smooth-solutions-in-hamiltonian-variational-ansatz/blob/fecd219024ff0b039a09030c723b860b7aa0186a/main_VQE_LTFIM_NewStartRand.py
import numpy as np
import sys
...
# output file
outfileOpt = f"temp_results_LTFIM/file_Opt_NewStartRand_r{randN}_LTFIM_g={g}_h={h}_{ans}_L={Lmin}_P={P}_maxiter={maxiter}_sign={sign}.npz"
...
en=(L)*result.optimal_value
resen=residual( ((L)*result.optimal_value), emin, emax)
fid=fidel(psi.to_matrix(massive= True),psi0)
optparlist=result.optimal_point
first_res=residual((L)*values[0],emin,emax)



np.savez(outfileOpt, en=en, resen=resen, fid=fid, optparlist=optparlist, first_res=first_res, emin=emin, emax=emax)
