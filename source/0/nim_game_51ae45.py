# https://github.com/qBraid/QCHack-2021/blob/8304761f577fa7f387369648b58b9d61d272d523/quantronauts/nim_game.py
import draw
import ia
    ...
    while stick > 0:
        ...
        if computer[0] is True and computer[1] is True:
            ...
            if stick == 1:
                ...
                if result_analyse == 4:
                    qc_board.h(stick - (1 + 0))
                    qc_board.cx(stick - 0, stick - (1 + 0))
                    qc_board.x(stick - (1 + 0))
                    drawing_add = ""
                    nbstick = 3
                    qc_board.h(stick - (1 + 1))
                    qc_board.cx(stick - 1, stick - (1 + 1))
                    qc_board.x(stick - (1 + 1))
                    drawing_add = ""
                    nbstick = 3
                    qc_board.h(stick - (1 + 2))
                    qc_board.cx(stick - 2, stick - (1 + 2))
                    qc_board.x(stick - (1 + 2))
                    drawing_add = ""
                    nbstick = 3
                else:
