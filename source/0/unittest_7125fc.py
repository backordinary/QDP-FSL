# https://github.com/Bmete7/QAOA-E/blob/e202bf177d122eaf6d5a1968f2c6d6c7e04ae49a/Test/unitTest.py
# -*- coding: utf-8 -*-
"""
...
import pickle
sys.path.append('..\\')
from Utils import KroneckerProductString, z, KroneckerProduct, efficientDiagonalKronecker
...
def TestCase(edges, adj, n = 3):
    ...
    for edge in edges:
        ...
        for step in range(n - 1):
            ...
            H += ((z1 + z2))
    return np.allclose(expm(H * -1j * beta) , U(beta), 10e-1), U(beta), expm(H * -1j * beta)
...
def TestKronecker(N):
    '''
    Checks if the diagonal sparse kronecker product method returns the correct operator
    ...
    for i in range(N):
        for j in range(N):
            if(i == j):
                continue
            print((efficientDiagonalKronecker([i,j] , z, N) == KroneckerProduct([i,j] , z , N)).all())
...
def main():
    ...
    if(type(edges) != list):
        raise TestFail('Wrong arguements passed')
    if(type(adj) != np.ndarray):
        raise TestFail('Wrong arguements passed')
    if(len(edges) <= 0):
        raise TestFail('Missing edges')

    if(TestCase2() == True):
        print('Encoding is done correctly!')
    else:
        raise TestFail('Encoding failed')
        
    
        
if __name__ == '__main__':
    # Instantiate the parser
    # parser = argparse.ArgumentParser(description='Number of qubits required')
