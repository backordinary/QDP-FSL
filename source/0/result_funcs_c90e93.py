# https://github.com/EeshGupta/VQE_Research/blob/98a0674eca203119b0c9ae358ce0379ba0a869fa/Coding%20Runs/Experiments/October/Gate%20Insertion%20and%20Complete%20VQE%20on%20H2%20(October%2014)/Result_funcs.py
import numpy as np
import random
...
def evaluator(circuits, samples, machine, noise_model, Hamiltonian, hammy_weights):
    """
...
def scaleEvaluator(scaled_circuits, samples, machine, noise_model, Hamiltonian, hammy_weights):
    '''
    Evaluate mean, std of vqe energies for diff scale factors
    '''
    means = []
    stds = []
    for scale_circs in scaled_circuits:
        print("Scale ----------")
        scale_energies = []
        for copy in scale_circs:
            energy = evaluator(copy, samples, machine, noise_model, Hamiltonian, hammy_weights)
            #print(energy)
            scale_energies.append(energy)
        means.append(np.mean(scale_energies))
        stds.append(np.std(scale_energies))
    return means, stds        