# https://github.com/Namr/YAQCS/blob/65a286f05fc917a39462d23ab1ad368f84466e7c/BQSKit%20Extensions/examples/ccx3.py
...
qc.rx(-np.pi/4,1)
qc.rx(np.pi/4,0)
qc.rxx(np.pi/4,2,0) #bug?? works but should be qc.rxx(np.pi/8,1,0)
qc.rxx(np.pi/4,1,0) #bug?? works but should be qc.rxx(np.pi/8,1,0)

qc.ry(np.pi/2,1)
qc.rx(-np.pi/2,1)
qc.rxx(-np.pi/2,2,1) #bug?? works but should be qc.rxx(np.pi/4,1,0)
