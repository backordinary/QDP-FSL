# https://github.com/kgeorgopoulos2/programbench/blob/d083b3e7f500dc599b53effdf02c5cc9cf9070fd/unm.py
# Import libraries
import numpy as np
...
#     colnames = ["Qubit", "Frequency", "T1", "T2", "SXError", "SQError", "ReadoutError", "TQError"]
    colnames = ["Qubit", "T1", "T2", "Frequency", "Anharmonicity", "ReadoutError", "StatePrepError0", "StatePrepError1", "ReadoutLength", "IDError", "SXError", "SQError", "TQError", "GateTime"]
...
def getSingleQubitErrorRates(data):
    '''Returns as a dictionary the single qubit error rates, as they appear on the machine. NOTE: the 
...
def getTwoQubitErrorRates(data):
    '''Returns as a dictionary the two qubit error rates, as they appear on the machine. NOTE: the values 
...
def getMeasureErrorRates(data):
    '''Returns as a dictionary the measurement error rates, as they appear on the machine. NOTE: the values
    deviate every time the machine gets callibrated'''
    qubits = data.Qubit.tolist()
    del qubits[0]
    qubits[0] = '0'
...
#     tqRates = getTwoQubitErrorRates() # Dictionary containing the two qubit error rates
    ...
    if (("sx" in line) or ("rz" in line) or ("id" in line) or (".x" in line)):
        s = line.split('[')
        s = s[1].split(']')
        qubit = ("Q" + s[0])
...
def thermalRelaxationChannel(backend, T1s, T2s, graph, gates):
    ...
    time_id = getSQGateExecutionTime('id', backend, gates) # Average of all id times
    time_rz = getSQGateExecutionTime('rz', backend, gates) # Average of all rz times
    time_x = getSQGateExecutionTime('x', backend, gates) # Average of all x times
    time_sx = getSQGateExecutionTime('sx', backend, gates) # Average of all sx times
    time_cx = getTQGateExecutionTime('cx', backend, graph) # Average of all cx times
    time_reset = 5048.889
    time_measure = 5048.889
...
def runUNM(path_data, graph, gates):
    '''Runs the Unified Noise Model. path_data: the location where the .csv file from the machine calibrations resides; graph: the two-qubit pairs that participate in the quantum circuit (i.e., [[0,1], [1,2], ...]); gates: the qubits that participate in the quantum circuit (i.e., [0, 1, ...]).'''
