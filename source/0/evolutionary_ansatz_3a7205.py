# https://github.com/aidanpellow/vqls/blob/ccb5584bb2991333ab9b8512371d29ff53090469/evolutionary_ansatz.py
import random
        ...
        if(initialize):
            ...
            if(init_with_gate_sequence != None):
                for i in range(len(init_with_gate_sequence)):
                    ...
                    if(init_with_gate_sequence[i] == 'CU3' or init_with_gate_sequence[i] == 'U3'):
                        self.n_gates+=1
                      
                if(random_parameters):
                    for i in range(3*self.n_gates):
            ...
            while(len(unassigned_gates) > 0):
                ...
                if(len(unassigned_gates) > 0 and random.randrange(0,2) == 0): #try CU3 first
                    control_index = random.choice(unassigned_gates)
                    if(previous_gate_sequence[chosen_index] != 'CU3' and previous_gate_sequence[control_index] != chosen_index):
                        chosen_gate = 'CU3'
                    ...
                    if(previous_gate_sequence[chosen_index] != 'U3'):
                        chosen_gate = 'U3'
                    else: #else try force CU3
                        ...
                        while(len(unchecked_controls) > 0):
                            ...
                            if(previous_gate_sequence[chosen_index] != 'CU3'):
                                chosen_gate = 'CU3'
                                unassigned_gates.remove(control_index)
                                break
                ...
                if(chosen_gate == 'CU3' or chosen_gate == 'U3'):                      
                    if(random_parameters):
                        self.parameters.append(random.random() * 2 * pi)
                    else:
                        self.parameters.append(0)
    ...
    def create_ansatz_individual(self, quantum_circuit, qubit_list): #draws the ansatz represented by the genome onto a circuit
        ...
        for ansatz_gene in self.ansatz_gene_list:
            ...
            for gate in gate_sequence:
                if(gate == "CU3"): #dealt with at control index
                    qubit_index += 1                
                elif(gate == "I"):
                    quantum_circuit.i(qubit_list[qubit_index])
                    qubit_index += 1
                elif(gate == "U3"):
                    quantum_circuit.u3(parameters[parameter_index], parameters[parameter_index+1], parameters[parameter_index+2], qubit_list[qubit_index])
                    qubit_index += 1
                    parameter_index += 3
                else:
                    controlled_gate_index = int(gate)
                    quantum_circuit.cu3(parameters[parameter_index], parameters[parameter_index+1], parameters[parameter_index+2], qubit_list[qubit_index], qubit_list[controlled_gate_index])
                    qubit_index += 1
                    parameter_index += 3 
