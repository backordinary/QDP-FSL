# https://github.com/Mohammad-Khatami/grover-protein-desing/blob/b946f36a0f398359da19bdfac962c280e1db8c07/codes/SP-model/6designable-3-4-5-interactions/6res-5inter-m7-iter-1-E16.py
#!/usr/bin/env python
...
for i in range(iteration):
    ...
    energy(qc_w,0,3,0,m,var_qubits,a_qubits)#bond C -> a_qubits  
    add(qc_w,a_qubits,b_qubits,o_qubits,m)# add A+B+C -> A
    energy_dg(qc_w,0,3,0,m,var_qubits,a_qubits)# clean A form a_qubits
    energy(qc_w,3,4,0,m,var_qubits,a_qubits)#bond C -> a_qubits  
    add(qc_w,a_qubits,b_qubits,o_qubits,m)# add A+B+C -> A
    energy_dg(qc_w,3,4,0,m,var_qubits,a_qubits)# clean A form a_qubits
    energy(qc_w,3,5,0,m,var_qubits,a_qubits)#bond C -> a_qubits  
    add(qc_w,a_qubits,b_qubits,o_qubits,m)# add A+B+C -> A
    energy_dg(qc_w,3,5,0,m,var_qubits,a_qubits)# clean A form a_qubits
    ##thrsh -16
    qc_w.x(a_qubits[4])
    ## now sum!
    ...
    ##thrsh -16
    qc_w.x(a_qubits[4])
    energy(qc_w,3,5,0,m,var_qubits,a_qubits)# clean A form a_qubits
    add_dg(qc_w,a_qubits,b_qubits,o_qubits,m)# add A+B+C -> A
    energy_dg(qc_w,3,5,0,m,var_qubits,a_qubits)#bond C -> a_qubits  
    energy(qc_w,3,4,0,m,var_qubits,a_qubits)# clean A form a_qubits
    add_dg(qc_w,a_qubits,b_qubits,o_qubits,m)# add A+B+C -> A
    energy_dg(qc_w,3,4,0,m,var_qubits,a_qubits)#bond C -> a_qubits  
    energy(qc_w,0,3,0,m,var_qubits,a_qubits)# clean A form a_qubits
    add_dg(qc_w,a_qubits,b_qubits,o_qubits,m)# add A+B+C -> A
    energy_dg(qc_w,0,3,0,m,var_qubits,a_qubits)#bond C -> a_qubits  
...
with open('6res-5inter-m7-iter-1-E16.txt', 'w') as file:
    file.write(str(data))
