# https://github.com/kterashi/QML_HEP/blob/52ae2681f291a4b694aaa9e71576041597fd7ee4/vqc_qc.py
...
plt.savefig('ROC_'+backend_name+'_'+str(feature_dim)+'d_'+str(training_size*2)+'evt_iter'+str(niter)+'_FMAPuin-depth'+str(uin_depth)+'_uvar-depth'+str(uvar_depth)+'_'+option+'.pdf')
...
plt.legend(loc="lower right")
plt.savefig('ROC2_'+backend_name+'_'+str(feature_dim)+'d_'+str(training_size*2)+'evt_iter'+str(niter)+'_FMAPuin-depth'+str(uin_depth)+'_uvar-depth'+str(uvar_depth)+'_'+option+'.pdf')
print(f'AUC(training) = {roc_auc_tr:.3f}')
print(f'AUC(testing) = {roc_auc:.3f}')
