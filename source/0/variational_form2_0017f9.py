# https://github.com/MarcDrudis/old_lattice/blob/78e6610c13d13bde0ad891c5b11312dc430f9d9a/lattice/wilson_fermions/variational_form2.py
...
def phase_like(site, lattice, S, copmat=np.array([[1,0],[0,1]]), output='qiskit'):
    ...
    summands = []
    summands.append(psidag(site, 0, lattice) * copmat[0, 0] * psi(site, 0,lattice))
    summands.append(psidag(site, 0, lattice) * copmat[0, 1] * psi(site, 1,lattice))
    summands.append(psidag(site, 1, lattice) * copmat[1, 0] * psi(site, 0,lattice))
    summands.append(psidag(site, 1, lattice) * copmat[1, 1] * psi(site, 1,lattice))
...
def hopping_like(edge, lattice, S, mixmat, hopp_coeff=1., output='qiskit'):
    ...
    bwd_hopp = (hopp_coeff
                * psidag(site, 0, lattice)
                * mixmat[0, 0]
                * psi(next_site, 0, lattice))
...
def string_like(edges, lattice, S, mixmat, hopp_coeff=1., output='qiskit'):
    ...
    # Generate the fermionic hopping terms
    bwd_hopp = (hopp_coeff
                * psidag(start_site, 0, lattice)
                * mixmat[0, 0]
                * psi(end_site, 0, lattice))
    fwd_hopp = bwd_hopp.dag()
