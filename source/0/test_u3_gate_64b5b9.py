# https://github.com/qudoor/qutrunk/blob/6d4b564c18d7a79ca6edcd2ad0ced1c1e385fe9a/qutrunk/test/gate/local/test_u3_gate.py
import numpy as np
import pytest
from qiskit import QuantumCircuit, assemble, Aer

from qutrunk.circuit.gates import H, All, U3
...
class TestU3:
    ...
    def test_matrix(self, result_qutrunk):
        """Test U3 gate with Matrix."""
        circuit = QCircuit()
        qr = circuit.allocate(1)
        Matrix(H.matrix.tolist()) * qr[0]
        Matrix(U3(np.pi, 0, np.pi).matrix.tolist()) * qr[0]
    ...
    def test_qiskit(self, result_qutrunk):
        """Test U3 gate with qiskit."""
        qc = QuantumCircuit(1)
        initial_state = [1, 0]
        qc.initialize(initial_state, 0)
        # apply gate
        qc.h(0)
        qc.u3(np.pi, 0, np.pi, 0)
        # run
    ...
    def test_matrix_inverse(self):
        """Test the inverse of U3 gate with Matrix."""
        circuit = QCircuit()
        qr = circuit.allocate(1)
        ...
        Matrix(U3(np.pi, 0, np.pi).matrix.tolist()) * qr[0]
        Matrix(U3(np.pi, 0, np.pi).matrix.tolist()).inv() * qr[0]
        result_m = circuit.get_statevector()
        result_m = np.array(result_m).reshape(-1, 1)
