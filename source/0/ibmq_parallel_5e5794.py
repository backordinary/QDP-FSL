# https://github.com/QQQYang/QUDIO/blob/1fd49a815a77fb647ae221c1cf54cfa763123237/ibmq_parallel.py
...
if __name__ == '__main__':
    ...
    print('The {}-th step: theta={}, loss={}'.format(0, [data[0].numpy() for data in res], [data[1] for data in res]))
    # params, losses = asyncio.run(multiple_tasks(theta))
    ...
    with Pool(4) as pool:
        res = pool.map(device, [(theta, 'ibmq_belem', h1), (theta, 'ibmq_bogota', h2), (theta, 'ibmq_quito', h3), (theta, 'ibmq_lima', h4)])
    theta = np.mean([data[0].numpy() for data in res])
    print('The {}-th step: theta={}, loss={}'.format(1, [data[0].numpy() for data in res], [data[1] for data in res]))
    # params, losses = asyncio.run(multiple_tasks(theta))
    ...
    with Pool(4) as pool:
        res = pool.map(device, [(theta, 'ibmq_belem', h1), (theta, 'ibmq_bogota', h2), (theta, 'ibmq_quito', h3), (theta, 'ibmq_lima', h4)])
    theta = np.mean([data[0].numpy() for data in res])
    print('The {}-th step: theta={}, loss={}'.format(2, [data[0].numpy() for data in res], [data[1] for data in res]))
    # params, losses = asyncio.run(multiple_tasks(theta))
    ...
    with Pool(4) as pool:
        res = pool.map(device, [(theta, 'ibmq_belem', h1), (theta, 'ibmq_bogota', h2), (theta, 'ibmq_quito', h3), (theta, 'ibmq_lima', h4)])
    theta = np.mean([data[0].numpy() for data in res])
    print('The {}-th step: theta={}, loss={}'.format(3, [data[0].numpy() for data in res], [data[1] for data in res]))
    # params, losses = asyncio.run(multiple_tasks(theta))
    ...
    with Pool(4) as pool:
        res = pool.map(device, [(theta, 'ibmq_belem', h1), (theta, 'ibmq_bogota', h2), (theta, 'ibmq_quito', h3), (theta, 'ibmq_lima', h4)])
    theta = np.mean([data[0].numpy() for data in res])
    print('The {}-th step: theta={}, loss={}'.format(4, [data[0].numpy() for data in res], [data[1] for data in res]))
    # params, losses = asyncio.run(multiple_tasks(theta))
    ...
    with Pool(4) as pool:
        res = pool.map(device, [(theta, 'ibmq_belem', h1), (theta, 'ibmq_bogota', h2), (theta, 'ibmq_quito', h3), (theta, 'ibmq_lima', h4)])
    theta = np.mean([data[0].numpy() for data in res])
    print('The {}-th step: theta={}, loss={}'.format(5, [data[0].numpy() for data in res], [data[1] for data in res]))
    # params, losses = asyncio.run(multiple_tasks(theta))
    ...
    with Pool(4) as pool:
        res = pool.map(device, [(theta, 'ibmq_belem', h1), (theta, 'ibmq_bogota', h2), (theta, 'ibmq_quito', h3), (theta, 'ibmq_lima', h4)])
    theta = np.mean([data[0].numpy() for data in res])
    print('The {}-th step: theta={}, loss={}'.format(6, [data[0].numpy() for data in res], [data[1] for data in res]))
    # params, losses = asyncio.run(multiple_tasks(theta))
    ...
    with Pool(4) as pool:
        res = pool.map(device, [(theta, 'ibmq_belem', h1), (theta, 'ibmq_bogota', h2), (theta, 'ibmq_quito', h3), (theta, 'ibmq_lima', h4)])
    theta = np.mean([data[0].numpy() for data in res])
    print('The {}-th step: theta={}, loss={}'.format(7, [data[0].numpy() for data in res], [data[1] for data in res]))
    # params, losses = asyncio.run(multiple_tasks(theta))
    ...
    with Pool(4) as pool:
        res = pool.map(device, [(theta, 'ibmq_belem', h1), (theta, 'ibmq_bogota', h2), (theta, 'ibmq_quito', h3), (theta, 'ibmq_lima', h4)])
    theta = np.mean([data[0].numpy() for data in res])
    print('The {}-th step: theta={}, loss={}'.format(8, [data[0].numpy() for data in res], [data[1] for data in res]))
    # params, losses = asyncio.run(multiple_tasks(theta))
    ...
    with Pool(4) as pool:
        res = pool.map(device, [(theta, 'ibmq_belem', h1), (theta, 'ibmq_bogota', h2), (theta, 'ibmq_quito', h3), (theta, 'ibmq_lima', h4)])
    theta = np.mean([data[0].numpy() for data in res])
    print('The {}-th step: theta={}, loss={}'.format(9, [data[0].numpy() for data in res], [data[1] for data in res]))
