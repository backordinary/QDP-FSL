# https://github.com/TheGupta2012/QPE-Algorithms/blob/feab65d74913236ce735db04c2446eb6a839616a/QPE/Modules/bundled_global_max_SPEA.py
...
class bundled_changed_SPEA:
    ...
    def get_circuits(self, angles, state):
        ...
        for theta in angles:
            ...
            qc = qc.compose(
                self.c_unitary_gate, qubits=range(1 + int(np.log2(self.dims)))
            )
    ...
    def get_eigen_pair(self, backend, progress=False, randomize=True):
        ...
        while 1 - cost >= precision:
            ...
            job_set = manager.run(
                circuits, backend=backend, name="Job_set" + str(iters), shots=shots
            )
            if progress:
