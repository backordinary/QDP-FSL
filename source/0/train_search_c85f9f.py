# https://github.com/yuxuan-du/Quantum_architecture_search/blob/429be34bd7ccce60453344a3e72f05fba07dde1e/machine_learning_large_memory/train_search.py
import argparse
...
def test_fn(model, subnet, expert_idx, inputs, labels):
    correct = 0
    for data, label in zip(inputs, labels):
        model.params = model.get_subnet_params(expert_idx, subnet)
        out = model(data, subnet)
...
def expert_evaluator(model, subnet, inputs, labels, n_experts):
    ...
    for i in range(n_experts):
        temp_loss = 0
        model.params = model.get_subnet_params(i, subnet)
        for data, label in zip(inputs, labels):
            out = model(data, subnet)
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        params = opt.step(lambda params: cost_fn(params, model, subnet, data_train, label_train, records), model.get_subnet_params(expert_idx, subnet)) # train
        model.set_subnet_params(params, expert_idx, subnet)
