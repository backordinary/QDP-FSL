# https://github.com/lucasponteslpa/QOCClassifier/blob/4a649c7d4ec6ecfae0bc965f7aabba1b2b974ed1/qclassifiers.py
import qiskit
import numpy as np
...
class QClassifier(object):
    ...
    def __init__(self, train_vectors, labels, test_vector, name='QOCC'):
        ...
        if name == 'HC':
            ...
            self.circuit = qiskit.QuantumCircuit(self.ancilla,self.ket_m,self.ket_phi,self.ket_y,self.c_anc,self.c_class)
        else:
            self.circuit = qiskit.QuantumCircuit(self.ancilla,self.ket_m,self.ket_phi,self.c_anc)

    def preparation(self):

        # Stage A: put the qubits of ancilla and ket_m register in superposition
        self.circuit.h(self.ancilla)
        for i in range(int(np.log2(self.train_vectors_size))):
            self.circuit.h(self.ket_m[i])
        ...
        self.circuit.append(q_test,list(qb))
        ...
        for i in range(self.train_vectors_size):
            ...
            if(self.log2N > 1):
                ...
                qb = np.append(qb,self.ket_phi)
            self.circuit.append(q_m,list(qb))
        ...
        if self.name == 'HC':
            for i in range(self.train_vectors_size):
                if self.target[i] == 1:
                    ...
                    self.circuit.append(target_gate,list(qb))


        self.circuit.h(self.ancilla)
        self.circuit.measure(self.ancilla,self.c_anc)
        if self.name=='HC':
            self.circuit.measure(self.ket_y,self.c_class)