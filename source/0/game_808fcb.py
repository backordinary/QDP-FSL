# https://github.com/yigithankardas/Q2048/blob/24b0837aadba134a0641ae9b58a872426356b801/game.py
import random
...
def checkLose():
    if cells[0][0].value == -256:
        elements = gameArea.winfo_children()
        elements[5].configure(text='You')
        elements[6].configure(text='Lost')
        return True
    if cells[0][1].value == -256:
        elements = gameArea.winfo_children()
        elements[5].configure(text='You')
...
def updateWidgets():
    ...
    for i in range(len(elements)):
        if cell_array[i].value != 0:
            ...
            if type(cell_array[i]) is Tile and cell_array[i].isInSuperposition:
                elements[i].configure(bg='#2F4F4F')
            else:
