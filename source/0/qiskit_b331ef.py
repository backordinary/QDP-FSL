# https://github.com/topher-lo/quantum-kitties/blob/dc5a10305de966546af0a276c47e63ab65e40cd7/game/qiskit.py
...
def randomize_pairs_ibmq():
    ...
    if flip[0] and (not (pairs) or 1 not in pairs[-1]):
        pairs.append((index1, 1 + 1))
    index1 = 2
    flip = random.sample([0, 1], 1)
    if flip[0] and (not (pairs) or 2 not in pairs[-1]):
        pairs.append((index1, 2 + 1))
    index1 = 3
    flip = random.sample([0, 1], 1)
    if flip[0] and (not (pairs) or 3 not in pairs[-1]):
        pairs.append((index1, 3 + 1))
    index1 = 4
    flip = random.sample([0, 1], 1)
    if flip[0] and (not (pairs) or 4 not in pairs[-1]):
        pairs.append((index1, 4 + 1))
    index1 = 5
    flip = random.sample([0, 1], 1)
    if flip[0] and (not (pairs) or 5 not in pairs[-1]):
        pairs.append((index1, 5 + 1))
    index1 = 6
    flip = random.sample([0, 1], 1)
    if flip[0] and (not (pairs) or 6 not in pairs[-1]):
        pairs.append((index1, 6 + 1))
    return pairs
