# https://github.com/kgeorgopoulos2/programbench/blob/d083b3e7f500dc599b53effdf02c5cc9cf9070fd/QASMParser.py
...
def QASMtoPython(path):
    ...
    for i in range(0,len(lines)):   
        ...
        elif ((("x" in lines[i]) or ('sx' in lines[i]) or ("id" in lines[i])) and ("cx" not in lines[i])):
            lines[i] = lines[i].replace(' ', '(')
            lines[i] = lines[i].replace(';', ')')
            lines[i] = "circ." + lines[i]
    ...
    for i in range(0,len(lines),2):
        if ("rz" in lines[i]):
            ...
            lines[i+1] = ("line = '" + lines[i] + "'" + "\r\n" + 
                          ...
                          "\r\t" + "else:" +
                          "\r\t\t" + "circ.z(q[" + q2 + "])")
        elif ((".x" in lines[i]) or ('sx' in lines[i]) or ("id" in lines[i])):
            s = lines[i].split('[')
            s = s[1].split(']')
            q = s[0]
            lines[i+1] = ("line = '" + lines[i] + "'" + "\r\n" + 
...
#     # Do the formatting of the file
#     for i in range(0,len(lines),2):
#         if (("rz" in lines[i]) or ("sx" in lines[i]) or ("id" in lines[i])):
#             s = lines[i].split('[')
#             s = s[1].split(']')
#             q = s[0]
#             lines[i+1] = ("line = '" + lines[i] + "'" + "\r\n" + 
...
def ErrorComposer(path):
    '''Composes the python code from the QASM file containing the dephasing error.'''
    file = QASMtoPython(path)
    AddQuantumErrors(path, file)
#     AddGAQuantumErrors(path, file)
