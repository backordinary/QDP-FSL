# https://github.com/Firepanda415/Matrix-Plots/blob/d0e83e84a31e5c3707e55b896db7c8f0b7f0b2b0/Scripts/measfilter.py
# -*- coding: utf-8 -*-
"""
...
plt.style.use(['seaborn-white', './Scripts/quantum-plots.mplstyle'])
...
#          (max_r, max_ind, d_ker(qs[max_ind]), qs_ker(qs[max_ind])))
    ...
    # lb = np.maximum(np.min(d), np.min(qs)) # lower boundary of pi^d/pi^Q(prior)
    # ub = np.maximum(np.max(d), np.max(qs)) # upper boundary of pi^d/pi^Q(prior)
