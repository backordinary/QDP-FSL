# https://github.com/yutakahirano/qc/blob/130abefe28c920f767e82bb9408e33c58766df17/simulation/simulator_test.py
import unittest
import numpy as np
import qiskit
import qulacs
...
class TestErrorGuessing(unittest.TestCase):
    ...
    def test_guessing_with_input_error(self):
        ...
        self.assertEqual(guessed_x_errors, ErrorSet({0}))
        self.assertEqual(guessed_z_errors, ErrorSet())
        self.assertEqual(x_errors, ErrorSet({0}))
        ...
        self.assertEqual(guessed_z_errors, ErrorSet({0}))
        self.assertEqual(x_errors, ErrorSet())
        self.assertEqual(z_errors, ErrorSet({0}))
        x_errors = ErrorSet({1})
        z_errors = ErrorSet()
        (guessed_x_errors, guessed_z_errors) = guess_errors(
            x_errors, z_errors, distribution)
        self.assertEqual(guessed_x_errors, ErrorSet({1}))
        self.assertEqual(guessed_z_errors, ErrorSet())
        self.assertEqual(x_errors, ErrorSet({1}))
        self.assertEqual(z_errors, ErrorSet())
        ...
        (guessed_x_errors, guessed_z_errors) = guess_errors(
            x_errors, z_errors, distribution)
        self.assertEqual(guessed_x_errors, ErrorSet())
        self.assertEqual(guessed_z_errors, ErrorSet({1}))
        self.assertEqual(x_errors, ErrorSet())
        self.assertEqual(z_errors, ErrorSet({1}))
        x_errors = ErrorSet({2})
        z_errors = ErrorSet()
        (guessed_x_errors, guessed_z_errors) = guess_errors(
            x_errors, z_errors, distribution)
        self.assertEqual(guessed_x_errors, ErrorSet({2}))
        self.assertEqual(guessed_z_errors, ErrorSet())
        self.assertEqual(x_errors, ErrorSet({2}))
        self.assertEqual(z_errors, ErrorSet())
        ...
        (guessed_x_errors, guessed_z_errors) = guess_errors(
            x_errors, z_errors, distribution)
        self.assertEqual(guessed_x_errors, ErrorSet())
        self.assertEqual(guessed_z_errors, ErrorSet({2}))
        self.assertEqual(x_errors, ErrorSet())
        self.assertEqual(z_errors, ErrorSet({2}))
        x_errors = ErrorSet({3})
        z_errors = ErrorSet()
        (guessed_x_errors, guessed_z_errors) = guess_errors(
            x_errors, z_errors, distribution)
        self.assertEqual(guessed_x_errors, ErrorSet({3}))
        self.assertEqual(guessed_z_errors, ErrorSet())
        self.assertEqual(x_errors, ErrorSet({3}))
        self.assertEqual(z_errors, ErrorSet())
        ...
        (guessed_x_errors, guessed_z_errors) = guess_errors(
            x_errors, z_errors, distribution)
        self.assertEqual(guessed_x_errors, ErrorSet())
        self.assertEqual(guessed_z_errors, ErrorSet({3}))
        self.assertEqual(x_errors, ErrorSet())
        self.assertEqual(z_errors, ErrorSet({3}))
        x_errors = ErrorSet({4})
        z_errors = ErrorSet()
        (guessed_x_errors, guessed_z_errors) = guess_errors(
            x_errors, z_errors, distribution)
        self.assertEqual(guessed_x_errors, ErrorSet({4}))
        self.assertEqual(guessed_z_errors, ErrorSet())
        self.assertEqual(x_errors, ErrorSet({4}))
        self.assertEqual(z_errors, ErrorSet())
        ...
        (guessed_x_errors, guessed_z_errors) = guess_errors(
            x_errors, z_errors, distribution)
        self.assertEqual(guessed_x_errors, ErrorSet())
        self.assertEqual(guessed_z_errors, ErrorSet({4}))
        self.assertEqual(x_errors, ErrorSet())
        self.assertEqual(z_errors, ErrorSet({4}))
        x_errors = ErrorSet({5})
        z_errors = ErrorSet()
        (guessed_x_errors, guessed_z_errors) = guess_errors(
            x_errors, z_errors, distribution)
        self.assertEqual(guessed_x_errors, ErrorSet({5}))
        self.assertEqual(guessed_z_errors, ErrorSet())
        self.assertEqual(x_errors, ErrorSet({5}))
        self.assertEqual(z_errors, ErrorSet())
        ...
        (guessed_x_errors, guessed_z_errors) = guess_errors(
            x_errors, z_errors, distribution)
        self.assertEqual(guessed_x_errors, ErrorSet())
        self.assertEqual(guessed_z_errors, ErrorSet({5}))
        self.assertEqual(x_errors, ErrorSet())
        self.assertEqual(z_errors, ErrorSet({5}))
        x_errors = ErrorSet({6})
        z_errors = ErrorSet()
        (guessed_x_errors, guessed_z_errors) = guess_errors(
            x_errors, z_errors, distribution)
        self.assertEqual(guessed_x_errors, ErrorSet({6}))
        self.assertEqual(guessed_z_errors, ErrorSet())
        self.assertEqual(x_errors, ErrorSet({6}))
        self.assertEqual(z_errors, ErrorSet())
        ...
        (guessed_x_errors, guessed_z_errors) = guess_errors(
            x_errors, z_errors, distribution)
        self.assertEqual(guessed_x_errors, ErrorSet())
        self.assertEqual(guessed_z_errors, ErrorSet({6}))
        self.assertEqual(x_errors, ErrorSet())
        self.assertEqual(z_errors, ErrorSet({6}))
