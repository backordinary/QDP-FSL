# https://github.com/EeshGupta/VQE_Research/blob/98a0674eca203119b0c9ae358ce0379ba0a869fa/Coding%20Runs/Experiments/November/Pulse%20and%20Gate%20Comparison%20(Nov10)/GateInsertion_funcs.py
import math
...
#     print('---------------------------------------')
    ...
    if 'pi' in new_angles[1]: 
        angle = PiReader(new_angles[1])
    else: 
        angle = float(new_angles[1])
        
        
    new_angles[1] = str(-math.pi - angle)
    
    if 'pi' in new_angles[2]: 
        angle = PiReader(new_angles[2])
    else: 
        angle = float(new_angles[2])
        
        
    new_angles[2] = str(-math.pi - angle)
#             if new_angles[i] == '-pi': 
...
def GateInserter(scale, circuits):
    scaled_circuits = []  
    
    for circ in circuits:
        #one is for zz, zi, etc and the other would be xx
        circ_ = circ.copy()
        scaled_circ = GateInsertion(circ_, scale)
        scaled_circuits.append(scaled_circ)
