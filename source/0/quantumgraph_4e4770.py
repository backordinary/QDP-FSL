# https://github.com/qiskit-community/QuantumGraph/blob/3c99ab256a3c5c6041399b3e1d6fdd2375e8ba96/quantumgraph/QuantumGraph.py
...
class QuantumGraph ():
    ...
    def set_bloch(self,target_expect,qubit,fraction=1,update=True):
        ...
        U[0][0] += target_basis[0][0]*conj(current_basis[0][0])
        U[0][1] += target_basis[0][0]*conj(current_basis[0][1])
        U[1][0] += target_basis[0][1]*conj(current_basis[0][0])
        U[1][1] += target_basis[0][1]*conj(current_basis[0][1])
        U[0][0] += target_basis[1][0]*conj(current_basis[1][0])
        U[0][1] += target_basis[1][0]*conj(current_basis[1][1])
        U[1][0] += target_basis[1][1]*conj(current_basis[1][0])
        U[1][1] += target_basis[1][1]*conj(current_basis[1][1])
    ...
    def set_relationship(self,relationships,qubit0,qubit1,fraction=1, update=True):
        ...
        vec = np.copy(vecs[2])
        vec -= inner(new_vecs[0],vec)*new_vecs[0]
        vec -= inner(new_vecs[1],vec)*new_vecs[1]
        while not valid[2]:
        ...
        U += outer(new_vecs[0],conj(vecs[0]))
        U += outer(new_vecs[1],conj(vecs[1]))
        U += outer(new_vecs[2],conj(vecs[2]))
        U += outer(new_vecs[3],conj(vecs[3]))
