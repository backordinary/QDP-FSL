# https://github.com/Jb-Za/QuantumTeleportationProtocol/blob/f517de797b42d3250deda62657c24b435858af11/QuantumTeleportation.py
...
def createCircuit(eight_bits):
    ...
    circuit.cz(7 , 7+16)
    circuit.barrier()
    circuit.measure(0 + 16, 0 ) # measure q2 and store into classical register 0
    circuit.measure(1 + 16, 1 ) # measure q2 and store into classical register 0
    circuit.measure(2 + 16, 2 ) # measure q2 and store into classical register 0
    circuit.measure(3 + 16, 3 ) # measure q2 and store into classical register 0
    circuit.measure(4 + 16, 4 ) # measure q2 and store into classical register 0
    circuit.measure(5 + 16, 5 ) # measure q2 and store into classical register 0
    circuit.measure(6 + 16, 6 ) # measure q2 and store into classical register 0
    circuit.measure(7 + 16, 7 ) # measure q2 and store into classical register 0
...
def executeCircuits(circuits):
    ...
    #print(counts)
    column = []
    for counts in marginal_counts(sim_result ,indices=[0]).get_counts(): # need to get the error values out of the resulted circuit execution
    ...
    #print(counts)
    column = []
    for counts in marginal_counts(sim_result ,indices=[1]).get_counts(): # need to get the error values out of the resulted circuit execution
        if str(1) in counts.keys() and str(0) in counts.keys(): # circuit can return {1 and 0} (some error) or {1} (no error) or {0} (no error). 
            if counts[str(1)] > counts[str(0)]:
                error_val = counts[str(0)]
                add_to_error(error_val)
    ...
    #print(counts)
    column = []
    for counts in marginal_counts(sim_result ,indices=[2]).get_counts(): # need to get the error values out of the resulted circuit execution
        if str(1) in counts.keys() and str(0) in counts.keys(): # circuit can return {1 and 0} (some error) or {1} (no error) or {0} (no error). 
            if counts[str(1)] > counts[str(0)]:
                error_val = counts[str(0)]
                add_to_error(error_val)
    ...
    #print(counts)
    column = []
    for counts in marginal_counts(sim_result ,indices=[3]).get_counts(): # need to get the error values out of the resulted circuit execution
        if str(1) in counts.keys() and str(0) in counts.keys(): # circuit can return {1 and 0} (some error) or {1} (no error) or {0} (no error). 
            if counts[str(1)] > counts[str(0)]:
                error_val = counts[str(0)]
                add_to_error(error_val)
    ...
    #print(counts)
    column = []
    for counts in marginal_counts(sim_result ,indices=[4]).get_counts(): # need to get the error values out of the resulted circuit execution
        if str(1) in counts.keys() and str(0) in counts.keys(): # circuit can return {1 and 0} (some error) or {1} (no error) or {0} (no error). 
            if counts[str(1)] > counts[str(0)]:
                error_val = counts[str(0)]
                add_to_error(error_val)
    ...
    #print(counts)
    column = []
    for counts in marginal_counts(sim_result ,indices=[5]).get_counts(): # need to get the error values out of the resulted circuit execution
        if str(1) in counts.keys() and str(0) in counts.keys(): # circuit can return {1 and 0} (some error) or {1} (no error) or {0} (no error). 
            if counts[str(1)] > counts[str(0)]:
                error_val = counts[str(0)]
                add_to_error(error_val)
    ...
    #print(counts)
    column = []
    for counts in marginal_counts(sim_result ,indices=[6]).get_counts(): # need to get the error values out of the resulted circuit execution
        if str(1) in counts.keys() and str(0) in counts.keys(): # circuit can return {1 and 0} (some error) or {1} (no error) or {0} (no error). 
            if counts[str(1)] > counts[str(0)]:
                error_val = counts[str(0)]
                add_to_error(error_val)
    ...
    #print(counts)
    column = []
    for counts in marginal_counts(sim_result ,indices=[7]).get_counts(): # need to get the error values out of the resulted circuit execution
        if str(1) in counts.keys() and str(0) in counts.keys(): # circuit can return {1 and 0} (some error) or {1} (no error) or {0} (no error). 
            if counts[str(1)] > counts[str(0)]:
                error_val = counts[str(0)]
                add_to_error(error_val)
