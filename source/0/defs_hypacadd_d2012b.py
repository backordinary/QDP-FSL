# https://github.com/hypahub/hypacadd_notebook/blob/f32b2ae545634efefa2de948ab3ed7b3773e4c6e/cQNN_qisQNN/defs_HypaCADD.py
...
def prep_plot_components_lr(X, y, X2, y2):
    ...
    scores = []
    clf = LogisticRegression(C=10**(1-0), penalty='l2', solver='liblinear')
    sc = cross_val_score(clf, X, y, cv=5)
    scores.append(sc.mean())
    clf = LogisticRegression(C=10**(1-1), penalty='l2', solver='liblinear')
    sc = cross_val_score(clf, X, y, cv=5)
    scores.append(sc.mean())
    clf = LogisticRegression(C=10**(1-2), penalty='l2', solver='liblinear')
    sc = cross_val_score(clf, X, y, cv=5)
    scores.append(sc.mean())
    clf = LogisticRegression(C=10**(1-3), penalty='l2', solver='liblinear')
    sc = cross_val_score(clf, X, y, cv=5)
    scores.append(sc.mean())
    clf = LogisticRegression(C=10**(1-4), penalty='l2', solver='liblinear')
    sc = cross_val_score(clf, X, y, cv=5)
    scores.append(sc.mean())
    clf = LogisticRegression(C=10**(1-5), penalty='l2', solver='liblinear')
    sc = cross_val_score(clf, X, y, cv=5)
    scores.append(sc.mean())
    clf = LogisticRegression(C=10**(1-6), penalty='l2', solver='liblinear')
    sc = cross_val_score(clf, X, y, cv=5)
    scores.append(sc.mean())
    clf = LogisticRegression(C=10**(1-7), penalty='l2', solver='liblinear')
    sc = cross_val_score(clf, X, y, cv=5)
    scores.append(sc.mean())
    clf = LogisticRegression(C=10**(1-8), penalty='l2', solver='liblinear')
    sc = cross_val_score(clf, X, y, cv=5)
    scores.append(sc.mean())
    clf = LogisticRegression(C=10**(1-9), penalty='l2', solver='liblinear')
    sc = cross_val_score(clf, X, y, cv=5)
    scores.append(sc.mean())
