# https://github.com/vutuanhai237/UC-VQA/blob/a913edd393a291b54afa9b7ae9dfed71157bbdcb/codes/tomography/multiprocessing_scripts/compare_WalltoallCNOT_qng.py
import qiskit
import numpy as np
import sys
...
def run_walltoall(num_layers, num_qubits):
    ...
    for i in range(0, 400):
        ...
        G = qtm.fubini_study.qng(qc.copy(), thetas, qtm.ansatz.create_WalltoallCNOT_layerd_state, num_layers)
        grad_loss = qtm.base.grad_loss(
            qc, 
            qtm.ansatz.create_WalltoallCNOT_layerd_state,
            thetas, num_layers = num_layers)
        thetas -= qtm.constant.learning_rate*(grad_loss) 
        thetass.append(thetas.copy())
        qc_copy = qtm.ansatz.create_WalltoallCNOT_layerd_state(qc.copy(), thetas, num_layers)  
    ...
    for thetas in thetass:
        qc = qiskit.QuantumCircuit(num_qubits, num_qubits)
        qc = qtm.ansatz.create_WalltoallCNOT_layerd_state(qc, thetas, num_layers = num_layers).inverse()
