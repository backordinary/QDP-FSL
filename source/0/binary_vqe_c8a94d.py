# https://github.com/ppravatto/Binary-VQE/blob/e16cd4965f7d3dbfcdb570237eec36f67004125b/binary_vqe.py
import os
from qiskit import *
...
class BIN_VQE():
    ...
    def run(self, method='Nelder-Mead', inital_parameters=[], max_iter=1000, tol=1e-5, verbose=False, filename=None, optimizer_options={}):
        ...
        if method == 'Nelder-Mead':
            ...
            _c = []
            label = "c" + str(0)
            _c.append(optimizer_options[label] if label in optimizer_options else default_spsa_c[0])
            label = "c" + str(1)
            _c.append(optimizer_options[label] if label in optimizer_options else default_spsa_c[1])
            label = "c" + str(2)
            _c.append(optimizer_options[label] if label in optimizer_options else default_spsa_c[2])
            label = "c" + str(3)
            _c.append(optimizer_options[label] if label in optimizer_options else default_spsa_c[3])
            label = "c" + str(4)
            _c.append(optimizer_options[label] if label in optimizer_options else default_spsa_c[4])
