# https://github.com/rsathyak/qml_masters_thesis/blob/02143908c3f2a5125b189d6af0dbff54436622c1/experiments/qmetrics/examples/qiskit/quantum-fourier-transform/qft_benchmark.py
"""
...
import qmetrics.metrics as metrics
...
def analyze_and_print_result(qc, result, num_qubits, secret_int, num_shots, method):
    ...
    if verbose:
        print(
            f"For secret int {secret_int} measured: {counts} fidelity: {fidelity}")
...
def run(min_qubits=2, max_qubits=8, max_circuits=3, num_shots=100,
    ...
    def execution_handler(qc, result, input_size, s_int, num_shots):
        ...
        counts, fidelity = analyze_and_print_result(
            qc, result, num_qubits, int(s_int), num_shots, method)
    ...
    for input_size in range(min_qubits, max_qubits + 1):
        ...
        if method == 1 or method == 2:
            ...
            if 2**(input_size) <= max_circuits:
                ...
                s_range = np.random.choice(
                    2**(input_size), num_circuits, False)
            ...
            if input_size <= max_circuits:
                ...
                s_range = np.random.choice(
                    range(input_size), num_circuits, False)
        ...
        for s_int in s_range:
            ...
            metrics.store_metric(
                input_size, s_int, 'create_time', time.time()-ts)
    ...
    metrics.plot_metrics(
        f"Benchmark Results - Variational Quantum Algorithm ({method}) - Qiskit")


# if main, execute method 1
# (min_qubits = 2, max_qubits = 8, max_circuits = 3, num_shots = 100,
#         method=1,
#         backend_id='qasm_simulator', provider_backend=None,
#         hub="ibm-q", group="open", project="main", exec_options=None)
if __name__ == '__main__':
    # run(2, 5, 3, 2048, 1, 'ibmq_lima')
    run()
