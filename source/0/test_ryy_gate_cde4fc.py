# https://github.com/qudoor/qutrunk/blob/6d4b564c18d7a79ca6edcd2ad0ced1c1e385fe9a/qutrunk/test/gate/local/test_ryy_gate.py
import pytest
import numpy as np
...
class TestRyyGate:
    ...
    def test_result_matrix(self, result_gate):
        ...
        Matrix(Ryy(pi / 2).matrix.tolist()) * (qr[0], qr[1])
    ...
    def test_matrix_inverse(self, backend):
        ...
        result_src = np.array(circuit.get_statevector()).reshape(-1, 1)
        Matrix(Ryy(pi / 2).matrix.tolist()) * (qr[0], qr[1])
        Matrix(Ryy(pi / 2).matrix.tolist()).inv() * (qr[0], qr[1])
        result_des = np.array(circuit.get_statevector()).reshape(-1, 1)
        assert np.allclose(result_src, result_des)