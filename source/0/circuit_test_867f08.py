# https://github.com/wugaxp/z-quantum-core/blob/6ab6bf502817b71c0eaa7ca92dd8df1854609400/src/python/zquantum/core/circuit/_circuit_test.py
import unittest
import numpy as np
import random
...
class TestCircuit(unittest.TestCase):
    ...
    def test_circuit_init(self):
        pyquil_prog = pyquil.quil.Program().inst(pyquil.gates.X(0),
                                        pyquil.gates.T(1),
    ...
    def test_zxz_cirq(self):
        ...
        cirq_circuit = cirq.Circuit()
        q = cirq.LineQubit(0)
        cirq_circuit.append([cirq_gate(q)], strategy=cirq.circuits.InsertStrategy.EARLIEST)
    ...
    def test_zxz_pyquil(self):
        ...
        circ1 = pyquil.quil.Program(RZ(-beta*pi, 0), RX(gamma*pi, 0), RZ(beta*pi, 0))
    ...
    def test_xx_cirq(self):
        ...
        U1 = Circuit(cirq2pyquil(cirq_circuit)).to_unitary()
        U2 = Circuit(cirq2pyquil(circuit3)).to_unitary()
        U3 = [[cos(beta*pi), 0, 0, -1j*sin(beta*pi)], [0, cos(beta*pi), -1j*sin(beta*pi), 0],
              [0, -1j*sin(beta*pi), cos(beta*pi), 0], [-1j*sin(beta*pi), 0, 0, cos(beta*pi)]]
    ...
    def test_xx_pyquil(self):
        ...
        #random.seed(RNDSEED)
        beta = random.uniform(-1,1) # rotation angles (in multiples of pi)
        circ1 = pyquil.quil.Program(H(0),H(1),CNOT(0,1),RZ(beta*2*pi,1),CNOT(0,1),H(0),H(1))
        ...
        u2 = Circuit(circ2).to_unitary()
        u3 = cirq_gate._unitary_()
        u4 = [[cos(beta*pi), 0, 0, -1j*sin(beta*pi)], [0, cos(beta*pi), -1j*sin(beta*pi), 0],
              [0, -1j*sin(beta*pi), cos(beta*pi), 0], [-1j*sin(beta*pi), 0, 0, cos(beta*pi)]]
    ...
    def test_yy_pyquil(self):
        ...
        #random.seed(RNDSEED)
        beta = random.uniform(-1,1) # rotation angles (in multiples of pi)
        circ1 = pyquil.quil.Program(S(0), S(1),\
    ...
    def test_zz_cirq(self):
        ...
        U1 = Circuit(cirq2pyquil(cirq_circuit)).to_unitary()
        U2 = Circuit(cirq2pyquil(circuit3)).to_unitary()
        U3 = [[cos(beta*pi)-1j*sin(beta*pi), 0, 0, 0], [0, cos(beta*pi)+1j*sin(beta*pi), 0, 0],
              [0, 0, cos(beta*pi)+1j*sin(beta*pi), 0], [0, 0, 0, cos(beta*pi)-1j*sin(beta*pi)]]
    ...
    def test_zz_pyquil(self):
        ...
        u2 = Circuit(circ2).to_unitary()
        u3 = cirq_gate._unitary_()
        u4 = [[cos(beta*pi)-1j*sin(beta*pi), 0, 0, 0], [0, cos(beta*pi)+1j*sin(beta*pi), 0, 0],
              [0, 0, cos(beta*pi)+1j*sin(beta*pi), 0], [0, 0, 0, cos(beta*pi)-1j*sin(beta*pi)]]
    ...
    def test_pyquil_conversion_general(self):
        ...
        onequbit_gates = [
            #'I',
            'X','Y','Z','H','S','T','RX','RY','RZ','PHASE']
        twoqubit_gates = ['CNOT','CZ',
    ...
    def test_cirq_conversion_on_qubits(self):
        ...
        self.assertEqual(is_identity(Circuit(test_circuit).to_unitary(), tol=1e-10), True)
    ...
    def test_cirq_conversion_general(self):
        ...
        onequbit_gates = ['X','Y','Z','H','S','T','Rx','Ry','Rz','PHASE','ZXZ', 'RH']
            ...
            for i in range(0, ngates):
                ...
                if gate in onequbit_gates:
                    ...
                    if gate in twoparam_gates:
                        if gate in {'ZXZ'}:
                            _gates.append(func(phase_exponent = theta, exponent = theta2)(cirq.LineQubit(q)))
                    if gate in oneparam_gates:
                        if gate in {'PHASE', 'RH'}:
                            func = func**theta
                ...
                if gate in twoqubit_gates:
                    ...
                    if gate in oneparam_gates:
                        theta = random.uniform(-1,1)
                        if gate in {'XX', 'YY', 'ZZ'}:
                            _gates.append(func(cirq.LineQubit(q1),cirq.LineQubit(q2))**theta)
                # if gate in threequbit_gates:
                #     qindices = random.sample(set(range(0, nqubits)),3)
                #     q1 = qindices[0]
                #     q2 = qindices[1]
        ...
        for _ in range(0, ntrials):
            for _ in range(0, ngates):
                ...
                if gate in twoqubit_gates:
                    ...
                    if gate in oneparam_gates:
                        theta = random.uniform(0,2*pi)
                        circ.append(func(theta), qargs=[list(qubits)[q1], list(qubits)[q2]])
                    if gate in noparam_gates:
    ...
    def test_cirq2pyquil_PhasedX(self): 
        circuit = cirq.Circuit()
        circuit.append(cirq.PhasedXPowGate(phase_exponent=0.25, exponent=0.5)(cirq.GridQubit(0, 0)))
        qprog = cirq2pyquil(circuit)
        self.assertEqual(qprog[0].name, 'RZ')
    ...
    def test_to_qpic(self):
        ...
        circuit.gates.append(Gate('CPHASE', [circuit.qubits[2], circuit.qubits[1]], [0.1]))
        circuit.gates.append(Gate('ZXZ', [circuit.qubits[2]], [np.pi/2, np.pi]))
        qpic_string = circuit.to_qpic()

        target_string = dedent("""w0 W 0
        w1 W 1
        w2 W 2
        w0 G {H} 
        w1 C w0
    ...
    def test_damping_gates(self):
        ...
        circuit = Circuit()
        circuit.qubits = qubits
        circuit.gates = [Gate('Z', [qubits[0]]),
                         Gate('Da', [qubits[1]], [p])]
        target_matrix = np.diag([1., np.sqrt(1  - p), -1., -np.sqrt(1 - p)])
