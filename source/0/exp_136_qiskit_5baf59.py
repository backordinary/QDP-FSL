# https://github.com/weiwenjiang/qc_mnist/blob/c337d72c207fad7b998528e77795cbf0bf1f8fba/Revision/Revision_Exp2-QF-FB/exp_136_qiskit.py
...
def q_map_q_ori_net(circ, s_qr_in, s_qr_enc, aux_qr, weights):
    if len(weights) == 4:
        ...
        flag = "03b"
        binstr_cur_num = format(0, flag)
        # print(binstr_cur_num)
        for j in range(len(binstr_cur_num)):
            if binstr_cur_num[j] == "0":
                circ.x(s_qr_enc[j])
        cccz(circ, s_qr_in[8 - 0 - 1], s_qr_enc[0], s_qr_enc[1], s_qr_enc[2], aux_qr[0], aux_qr[1])
        for j in range(len(binstr_cur_num)):
            if binstr_cur_num[j] == "0":
                circ.x(s_qr_enc[j])
        circ.barrier()
        binstr_cur_num = format(1, flag)
        # print(binstr_cur_num)
        for j in range(len(binstr_cur_num)):
            if binstr_cur_num[j] == "0":
                circ.x(s_qr_enc[j])
        cccz(circ, s_qr_in[8 - 1 - 1], s_qr_enc[0], s_qr_enc[1], s_qr_enc[2], aux_qr[0], aux_qr[1])
        for j in range(len(binstr_cur_num)):
            if binstr_cur_num[j] == "0":
                circ.x(s_qr_enc[j])
        circ.barrier()
        binstr_cur_num = format(2, flag)
        # print(binstr_cur_num)
        for j in range(len(binstr_cur_num)):
            if binstr_cur_num[j] == "0":
                circ.x(s_qr_enc[j])
        cccz(circ, s_qr_in[8 - 2 - 1], s_qr_enc[0], s_qr_enc[1], s_qr_enc[2], aux_qr[0], aux_qr[1])
        for j in range(len(binstr_cur_num)):
            if binstr_cur_num[j] == "0":
                circ.x(s_qr_enc[j])
        circ.barrier()
        binstr_cur_num = format(3, flag)
        # print(binstr_cur_num)
        for j in range(len(binstr_cur_num)):
            if binstr_cur_num[j] == "0":
                circ.x(s_qr_enc[j])
        cccz(circ, s_qr_in[8 - 3 - 1], s_qr_enc[0], s_qr_enc[1], s_qr_enc[2], aux_qr[0], aux_qr[1])
        for j in range(len(binstr_cur_num)):
            if binstr_cur_num[j] == "0":
                circ.x(s_qr_enc[j])
        circ.barrier()
        binstr_cur_num = format(4, flag)
        # print(binstr_cur_num)
        for j in range(len(binstr_cur_num)):
            if binstr_cur_num[j] == "0":
                circ.x(s_qr_enc[j])
        cccz(circ, s_qr_in[8 - 4 - 1], s_qr_enc[0], s_qr_enc[1], s_qr_enc[2], aux_qr[0], aux_qr[1])
        for j in range(len(binstr_cur_num)):
            if binstr_cur_num[j] == "0":
                circ.x(s_qr_enc[j])
        circ.barrier()
        binstr_cur_num = format(5, flag)
        # print(binstr_cur_num)
        for j in range(len(binstr_cur_num)):
            if binstr_cur_num[j] == "0":
                circ.x(s_qr_enc[j])
        cccz(circ, s_qr_in[8 - 5 - 1], s_qr_enc[0], s_qr_enc[1], s_qr_enc[2], aux_qr[0], aux_qr[1])
        for j in range(len(binstr_cur_num)):
            if binstr_cur_num[j] == "0":
                circ.x(s_qr_enc[j])
        circ.barrier()
        binstr_cur_num = format(6, flag)
        # print(binstr_cur_num)
        for j in range(len(binstr_cur_num)):
            if binstr_cur_num[j] == "0":
                circ.x(s_qr_enc[j])
        cccz(circ, s_qr_in[8 - 6 - 1], s_qr_enc[0], s_qr_enc[1], s_qr_enc[2], aux_qr[0], aux_qr[1])
        for j in range(len(binstr_cur_num)):
            if binstr_cur_num[j] == "0":
                circ.x(s_qr_enc[j])
        circ.barrier()
        binstr_cur_num = format(7, flag)
        # print(binstr_cur_num)
        for j in range(len(binstr_cur_num)):
            if binstr_cur_num[j] == "0":
                circ.x(s_qr_enc[j])
        cccz(circ, s_qr_in[8 - 7 - 1], s_qr_enc[0], s_qr_enc[1], s_qr_enc[2], aux_qr[0], aux_qr[1])
        for j in range(len(binstr_cur_num)):
            if binstr_cur_num[j] == "0":
                circ.x(s_qr_enc[j])
        circ.barrier()
