# https://github.com/a0610328/QuantumMachineLearning/blob/f9e1e167d1b3cf3f7285132919b3ea32a025f48b/QuantumCircuitLearning2.py
# -*- coding: utf-8 -*-
"""
...
# =============================================================================
    ...
    pred = qcl.predictions(X_test_plot[:,0].reshape(-1,2), theta)
    Y_test_plot[:,0] = np.array(list(map(qcl.softmax, pred)))[:,0]
    pred = qcl.predictions(X_test_plot[:,1].reshape(-1,2), theta)
    Y_test_plot[:,1] = np.array(list(map(qcl.softmax, pred)))[:,0]
    pred = qcl.predictions(X_test_plot[:,2].reshape(-1,2), theta)
    Y_test_plot[:,2] = np.array(list(map(qcl.softmax, pred)))[:,0]
    pred = qcl.predictions(X_test_plot[:,3].reshape(-1,2), theta)
    Y_test_plot[:,3] = np.array(list(map(qcl.softmax, pred)))[:,0]
    pred = qcl.predictions(X_test_plot[:,4].reshape(-1,2), theta)
    Y_test_plot[:,4] = np.array(list(map(qcl.softmax, pred)))[:,0]
    pred = qcl.predictions(X_test_plot[:,5].reshape(-1,2), theta)
    Y_test_plot[:,5] = np.array(list(map(qcl.softmax, pred)))[:,0]
    pred = qcl.predictions(X_test_plot[:,6].reshape(-1,2), theta)
    Y_test_plot[:,6] = np.array(list(map(qcl.softmax, pred)))[:,0]
    pred = qcl.predictions(X_test_plot[:,7].reshape(-1,2), theta)
    Y_test_plot[:,7] = np.array(list(map(qcl.softmax, pred)))[:,0]
    pred = qcl.predictions(X_test_plot[:,8].reshape(-1,2), theta)
    Y_test_plot[:,8] = np.array(list(map(qcl.softmax, pred)))[:,0]
    pred = qcl.predictions(X_test_plot[:,9].reshape(-1,2), theta)
    Y_test_plot[:,9] = np.array(list(map(qcl.softmax, pred)))[:,0]
