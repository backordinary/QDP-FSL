# https://github.com/zademi/QI_project_ichverzweifel/blob/2066ac99d208b29dbc6384f3dd6e8ea31dde5381/hardware%20error%20correction.py
...
counts_ancilla_first_round = marginal_counts(simp_job.result(), indices=[0, 1]).get_counts()
print("Results_ancilla: ", counts_ancilla_first_round)
# plot_histogram(counts_ancilla, title='Ancilla measurements after {} rounds'.format(n_rounds))
plot_histogram(counts_ancilla_first_round, title='Ancilla measurements after the first round')
plt.show()

counts_ancilla_last_round = marginal_counts(simp_job.result(), indices=[x - 2, x - 1]).get_counts()
print("Results_ancilla: ", counts_ancilla_last_round)
plot_histogram(counts_ancilla_last_round, title='Ancilla measurements after {} rounds'.format(n_rounds))
# plot_histogram(counts_ancilla, title='Ancilla measurements after the first round')
plt.show()
