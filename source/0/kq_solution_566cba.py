# https://github.com/quantum-melbourne/mcds-workshop-21/blob/feb58550a7da058ef0135d2f62bd0f1b16ec9f62/day-three/usman-coreset-examples/kq_solution.py
"""
...
#topology for ibmq_guadalupe, is required to run the code
coupling_map = [[0, 1],
 [1, 0],
 [1, 2],
 [1, 4],
...
def gen_coreset_graph(coreset=None, metric='dot',load = True):
    """
    ...
    """
    if load == True:
        coreset=np.load('iris_coreset.npy',allow_pickle=True) 
    else:
        coreset = coreset
    if coreset is None:
    ...
    if load == True:
        coreset[0][1][0] = coreset[0][1][0]/1000
        coreset[0][1][1] = coreset[0][1][1]/1000
        coreset[0][1][2] = coreset[0][1][2]/1000
        coreset[0][1][3] = coreset[0][1][3]/1000
        coreset[1][1][0] = coreset[1][1][0]/1000
        coreset[1][1][1] = coreset[1][1][1]/1000
        coreset[1][1][2] = coreset[1][1][2]/1000
        coreset[1][1][3] = coreset[1][1][3]/1000
        coreset[2][0] = coreset[2][0]/10
    return coreset, G, H
...
def optimize_qaoa(init_params, num_params, shots, P, G, topology, device=None,
                  delta=1.7, tol=10e-12, no_improv_break=15, max_iter=5000):
    """
    """
    def f(params):
        gamma, beta = params
        ...
        if len(topology) > 0:
            circ,_= gen_complete_qaoa_circ(P, [gamma], [beta], G,topology=topology)
        
        else:                                 
            circ= gen_complete_qaoa_circ(P, [gamma], [beta], G,topology=topology)
    ...
    opt_params, opt_cost = minimizeEnergyObjective(f, init_params, num_params,delta, tol, no_improv_break, max_iter)
...
def gen_complete_qaoa_circ_no_measure(P, gamma, beta, G, ising=False, topology=[]):
    """
...
def qaoa(H,P,G):
    ...
    hstr = hstr[:-2]
    num_params = 2
    gamma, beta = [3.58635686,5.47179902]
    circuit = gen_complete_qaoa_circ(P, [gamma], [beta], G, ising=False, topology=[])
    print(circuit.draw(output='text', fold=100))
    backend_pure = Aer.get_backend('qasm_simulator')
    result = execute(circuit, backend_pure).result()
