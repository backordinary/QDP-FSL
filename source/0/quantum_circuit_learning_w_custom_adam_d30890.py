# https://github.com/a0610328/QuantumMachineLearning/blob/8b580072f5e0c50d1d35fc2c173e0c81fd2170e7/quantum_circuit_learning_w_custom_adam.py
# -*- coding: utf-8 -*-
"""
...
def adam(X_dat, Y_dat, minibatch_size = None, maxiter = 50):
    ...
    for i in range(maxiter):
        for j in tqdm(range(len(X_batched))):
            ...
            derivative = gradient(X_minibatch, Y_minibatch, theta)
            m = beta_1 * m + (1 - beta_1) * derivative     #TODO, check dimension
...
theta = adam(X_dat, Y_dat, minibatch_size, maxiter)
t2 = time()
...
pred = predictions(X_test_plot[:,0].reshape(-1,2), theta)
Y_test_plot[:,0] = np.array(list(map(softmax, pred)))[:,0]
pred = predictions(X_test_plot[:,1].reshape(-1,2), theta)
Y_test_plot[:,1] = np.array(list(map(softmax, pred)))[:,0]
pred = predictions(X_test_plot[:,2].reshape(-1,2), theta)
Y_test_plot[:,2] = np.array(list(map(softmax, pred)))[:,0]
pred = predictions(X_test_plot[:,3].reshape(-1,2), theta)
Y_test_plot[:,3] = np.array(list(map(softmax, pred)))[:,0]
pred = predictions(X_test_plot[:,4].reshape(-1,2), theta)
Y_test_plot[:,4] = np.array(list(map(softmax, pred)))[:,0]
pred = predictions(X_test_plot[:,5].reshape(-1,2), theta)
Y_test_plot[:,5] = np.array(list(map(softmax, pred)))[:,0]
pred = predictions(X_test_plot[:,6].reshape(-1,2), theta)
Y_test_plot[:,6] = np.array(list(map(softmax, pred)))[:,0]
pred = predictions(X_test_plot[:,7].reshape(-1,2), theta)
Y_test_plot[:,7] = np.array(list(map(softmax, pred)))[:,0]
pred = predictions(X_test_plot[:,8].reshape(-1,2), theta)
Y_test_plot[:,8] = np.array(list(map(softmax, pred)))[:,0]
pred = predictions(X_test_plot[:,9].reshape(-1,2), theta)
Y_test_plot[:,9] = np.array(list(map(softmax, pred)))[:,0]
