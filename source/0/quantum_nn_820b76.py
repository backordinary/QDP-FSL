# https://github.com/rsathyak/qml_masters_thesis/blob/dd9ceb00a462e48258a1a3570e2ab59d8bb463cf/experiments/qmetrics/qmetrics/effective_dimension/quantum_nn.py
import numpy as np
...
class QuantumNeuralNetwork(Model):
    ...
    def get_probs(self, inds, thetas, datas, circuit, results):
    ...
    def forward(self, params: Union[List, np.ndarray], x: Union[List, np.ndarray]):
        ...
        processes = [mp.Process(target=self.get_probs, args=(inds, params[inds], x[inds], self.circuit, results))
                     for inds in indices]
