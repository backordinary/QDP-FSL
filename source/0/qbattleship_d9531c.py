# https://github.com/dsaf2007/QuantumComputing/blob/21f0a08bcd398235e0f9f54120dc7d7e910e9498/eclipseWorkspace/BattleShip/WebContent/python/qbattleship.py
# Quantum Battleship
...
for n in range(bomb[(0+1) % 2][0]):
    for ship in [0, 1, 2]:
        if (0 == shipPos[0][ship]):
            frac = 1/(ship+1)
            gridScript.u(frac * math.pi, 0.0, 0.0, q[0]) 
		# add as many bombs as have been placed at this position
for n in range(bomb[(0+1) % 2][1]):
    for ship in [0, 1, 2]:
        if (1 == shipPos[0][ship]):
            frac = 1/(ship+1)
            gridScript.u(frac * math.pi, 0.0, 0.0, q[1]) 
		# add as many bombs as have been placed at this position
for n in range(bomb[(0+1) % 2][2]):
    for ship in [0, 1, 2]:
        if (2 == shipPos[0][ship]):
            frac = 1/(ship+1)
            gridScript.u(frac * math.pi, 0.0, 0.0, q[2]) 
		# add as many bombs as have been placed at this position
for n in range(bomb[(0+1) % 2][3]):
    for ship in [0, 1, 2]:
        if (3 == shipPos[0][ship]):
            frac = 1/(ship+1)
            gridScript.u(frac * math.pi, 0.0, 0.0, q[3]) 
		# add as many bombs as have been placed at this position
for n in range(bomb[(0+1) % 2][4]):
    for ship in [0, 1, 2]:
        if (4 == shipPos[0][ship]):
            frac = 1/(ship+1)
            gridScript.u(frac * math.pi, 0.0, 0.0, q[4]) 
