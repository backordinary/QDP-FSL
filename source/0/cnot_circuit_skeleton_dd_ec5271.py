# https://github.com/siddharthdangwal/ADAPT/blob/0740a5b4e5ada53dd31d59cc647741998563dc77/scripts/cnot_circuit_skeleton_dd.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
## Revision Notes by PD:
## check_and_apply function changed - inserted barriers for XYXY sequence to prevent crazy optimizations, added two versions of IBMQ DD sequence (with identities and with delay slots)
## added additional modes for DD sequence insertion and merged those changes to all locations where DD could be inserted  
## picked up SD's bug fixes
...
def check_and_apply(qc, idx, dd_time, gate_lengths, tm,dd_type='xyxy'):
    ...
    if dd_type == 'xx':
        ...
        #print('Per slot delay duration ', per_delay_slot_duration, ' for DD type ', dd_type)
        #print('For this set up : delay time ', delay_duration, ' dd sequence length ', dd_seq_length, ' total dd time ', dd_time)
            ...
            if mode in dd_modes: 
                ...
                for qb in range(n_qubits):
                    ...
                    if (int(tab[qubit_val]) != int(min_free_evolution_tab)):
                        ...
                        for _curr_qubit in qubits_to_consider:
                            if qubit_val!=_curr_qubit:
                                qc = check_and_apply(qc, _curr_qubit, dd_time, gate_lengths, tm, mode)
