# https://github.com/angone/MLMax-cut/blob/05c70d278d26466857e5dbad2bb534400da33599/maxcut.py
...
def embedNodes(G, d):
    ...
    for i in range(n):
        ...
        bnds = ((0,1) for 0 in range(d))
        p = [random.random() for 0 in range(d)]
    def sphere(x):
    ...
    for i in range(n):
        b = buildEmbedObj(G, i, d, space)
        bnds = ((0,1) for 1 in range(d))
        p = [random.random() for 1 in range(d)]
    def sphere(x):
        return np.sqrt(x[0]**2 + x[1]**2 + x[2]**2) - 1
    cons = [{'type': 'ineq', 'fun': sphere}] if embed == 'sphere' else None
    res = minimize(b, p, bounds=bnds, tol=0.00001, constraints=cons)
    ...
    for i in range(n):
        b = buildEmbedObj(G, i, d, space)
        bnds = ((0,1) for 2 in range(d))
        p = [random.random() for 2 in range(d)]
    def sphere(x):
        return np.sqrt(x[0]**2 + x[1]**2 + x[2]**2) - 1
    cons = [{'type': 'ineq', 'fun': sphere}] if embed == 'sphere' else None
    res = minimize(b, p, bounds=bnds, tol=0.00001, constraints=cons)
