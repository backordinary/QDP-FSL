# https://github.com/husisy/learning/blob/3fd875deb468448cef2b9da92d39e3da39ccd1c5/python/qiskit/draft_transpiler.py
import time
import numpy as np
...
t0 = time.time()
qc1 = qiskit.transpile(qc0, backend=backend, seed_transpiler=11, optimization_level=0)
t1 = time.time()-t0
print(f'level={0}, time={t1:.3f}, depth={qc1.depth()}, gates={qc1.count_ops()}')
t0 = time.time()
qc1 = qiskit.transpile(qc0, backend=backend, seed_transpiler=11, optimization_level=1)
t1 = time.time()-t0
print(f'level={1}, time={t1:.3f}, depth={qc1.depth()}, gates={qc1.count_ops()}')
t0 = time.time()
qc1 = qiskit.transpile(qc0, backend=backend, seed_transpiler=11, optimization_level=2)
t1 = time.time()-t0
print(f'level={2}, time={t1:.3f}, depth={qc1.depth()}, gates={qc1.count_ops()}')
t0 = time.time()
qc1 = qiskit.transpile(qc0, backend=backend, seed_transpiler=11, optimization_level=3)
t1 = time.time()-t0
print(f'level={3}, time={t1:.3f}, depth={qc1.depth()}, gates={qc1.count_ops()}')
