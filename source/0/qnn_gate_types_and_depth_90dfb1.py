# https://github.com/maiyuren/Hybrid-Quantum-Autoencoder/blob/4c176519b0f0fe1a644beafedc0c7754bdb65435/required_modules/qnn_gate_types_and_depth.py
...
def barren_script():
    ...
    all_arguments = []
    for num_params in range(4, 16*4, 4):
        distr = normalise(gaussian_distr(2 ** 4, std_dev=3, mean=0))
        all_arguments.append((N, 4, gate, num_params, bitflip, phaseflip, criterion, distr, num_iterations, step))
    for num_params in range(5, 16*5, 5):
        distr = normalise(gaussian_distr(2 ** 5, std_dev=3, mean=0))
        all_arguments.append((N, 5, gate, num_params, bitflip, phaseflip, criterion, distr, num_iterations, step))
    for num_params in range(6, 16*6, 6):
        distr = normalise(gaussian_distr(2 ** 6, std_dev=3, mean=0))
        all_arguments.append((N, 6, gate, num_params, bitflip, phaseflip, criterion, distr, num_iterations, step))
    for num_params in range(7, 16*7, 7):
        distr = normalise(gaussian_distr(2 ** 7, std_dev=3, mean=0))
        all_arguments.append((N, 7, gate, num_params, bitflip, phaseflip, criterion, distr, num_iterations, step))
