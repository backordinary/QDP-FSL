# https://github.com/Serock3/Master_thesis_QEC_simulation/blob/bfc808d5e73b7e0bc77f7a36bdc373f7d27a4c8f/simulator_program/stabilizers_422.py
...
def stabilizer_cycle_422(registers, reset=True, current_cycle=0,
    ...
    circ = get_empty_stabilizer_circuit_422(registers)
    circ.compose(stabilizer_list[0](registers,
                                    syn_bit=syn_bit_list[0], reset=reset), inplace=True)
    if extra_snapshots:
        add_snapshot_to_circuit(circ, qubits=registers.QubitRegister,
                                include_barriers=include_barriers, **kwargs)
    elif include_barriers:
        circ.barrier()
    circ.compose(stabilizer_list[1](registers,
                                    syn_bit=syn_bit_list[1], reset=reset), inplace=True)
    if extra_snapshots:
        add_snapshot_to_circuit(circ, qubits=registers.QubitRegister,
                                include_barriers=include_barriers, **kwargs)
    elif include_barriers:
