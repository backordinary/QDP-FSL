# https://github.com/braqiiit/QRealBoost/blob/09b364f55c08e18214871fe928cbe0cbd9ae9179/Preliminary%20Experiments/Breast_Cancer_Dataset/m_32/QReal_Boost_cancer_M32_Q4.py
#!/usr/bin/env python
# coding: utf-8
...
def check_j():
    ...
    '''
    used for checking if the value of k and jti are same or not 
    and then on the basis of that setting the auxilary qubit as True or False
    '''
...
def check_y():
    ...
    '''
    used for checking if the value of y and b is same or not 
    and then on the basis of that setting the auxilary qubit as True or False
    '''
...
def new_qc( mc, data):
    ...
    '''
    Unitary which is able to produce a number 
    encoded in the binary format for production of k and b
...
def numConcat(num1, num2):
    ...
    '''
    this does additions of inputs in the form of strings
    the given output stored in the matrix has to be converted to string
...
def dec_to_bin(n, size):
    ...
    '''
    just another function for converting from decimal to binary
    '''    
...
def f2bin(number,places):
    ...
    '''
    for converting decimal fractions (floating point) to binary
...
# 
# Find more details regarding this data set at sklearn documentation; load_breast_cancer()
...
## testing data 
test_size = 32
X_test = sample_train[i+train_size:i+train_size + test_size]
y_test = label_train[i+train_size:i+train_size + test_size]
...
def original_acc(X,y, no_of_Q):
    ...
    '''
    Function is created for checking the accuracy of the classifier in the first iteration
    '''
...
def amplification( Dti, reps):
    ...
    '''
    this will be able to actually be able to create the amplification circuit, 
    run it and obtain the values of Dti after the amplification has been done
    '''    
...
def A(y ,k , b, preds, Dti):
    ...
    '''
    This circuit will estitmate the partition label weigths (the Wkb's). 
    '''
...
def iteration_iqae( y, preds,Dti):
    ...
    '''
    This function executes the cirucit given above and uses IQAE on it to get the values of W+ and W- 
    Once these are obtained it returns the value of beta's 
    '''
...
def complete_imp3(num_iterations,X,test_data,y):
    ...
    '''
    The function which puts all the above functions together 
    in order to obtain the accuracy and classifiers for the testing 
