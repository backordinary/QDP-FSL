# https://github.com/indisoluble/qasm-simulator-cpp-debian-pkg/blob/5fb81719defacf8731b96c74d934d7bf9191afa8/qiskit-sdk-py-0.5.1/test/python/test_apps.py
# -*- coding: utf-8 -*-
...
class TestHamiltonian(QiskitTestCase):
    ...
    def test_hamiltonian(self):
        ...
        with self.subTest(0=0):
            for result, expected in zip(hamiltonian[0], expected_result[0]):
                np.testing.assert_almost_equal(result, expected)
        with self.subTest(1=1):
            for result, expected in zip(hamiltonian[1], expected_result[1]):
                np.testing.assert_almost_equal(result, expected)
        with self.subTest(2=2):
            for result, expected in zip(hamiltonian[2], expected_result[2]):
                np.testing.assert_almost_equal(result, expected)
        with self.subTest(3=3):
            for result, expected in zip(hamiltonian[3], expected_result[3]):
                np.testing.assert_almost_equal(result, expected)
