# https://github.com/EeshGupta/VQE_Research/blob/98a0674eca203119b0c9ae358ce0379ba0a869fa/Coding%20Runs/Experiments/November/Inverted%20and%20Normal%20Schnetzer%20Readout%20Method%20(November%208%202020)/Result_ReadoutMit_funcs.py
import numpy as np
import random
...
def prepare_circuit(circ, invert = False):
    '''
...
def modify_counts(counts, shots, invert = False):
    '''
...
def evaluator(circuits, samples, noise_model, Hamiltonian, hammy_weights, simulator, readout_mit, invert= False):
    """
    ...
    for i_circ in range(len(circuits)):
        ...
        if(noise_model==None):
            ...
            if(readout_mit == False):
                ...
                circ = prepare_circuit(circ, invert)
                #print('hi')
                #print(circ.qasm())
                job = execute(circ, backend = simulator, noise_model = noise_model,shots = 8192)
                counts = job.result().get_counts()
                counts = modify_counts(counts, 8192, invert)
            #print(counts)
