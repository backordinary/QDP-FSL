# https://github.com/quantum-compiler/quartz-artifact/blob/dfc64946c7d7ef016691c45bece6960df661cfac/Qiskit_ibmq.py
...
def qiskit_optimize_nam(fn):
    ...
    optimize_times = []
    start = time.time()
    optimized_circ = transpile(circ, basis_gates=['u1', 'u2', 'u3', 'cx'], optimization_level=0)
    ...
    start = time.time()
    optimized_circ = transpile(circ, basis_gates=['u1', 'u2', 'u3', 'cx'], optimization_level=1)
    optimize_times.append(time.time() - start)
    count_dict = optimized_circ.count_ops()
    gate_cnts.append(sum([count_dict[k] for k in count_dict]))
    start = time.time()
    optimized_circ = transpile(circ, basis_gates=['u1', 'u2', 'u3', 'cx'], optimization_level=2)
    optimize_times.append(time.time() - start)
    count_dict = optimized_circ.count_ops()
    gate_cnts.append(sum([count_dict[k] for k in count_dict]))
