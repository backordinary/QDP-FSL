# https://github.com/Serock3/Master_thesis_QEC_simulation/blob/bfc808d5e73b7e0bc77f7a36bdc373f7d27a4c8f/trash/decoder_testing.py
...
def unflagged_stabilizer_cycle(registers, reset=True, recovery=False,
    ...
    circ = get_empty_stabilizer_circuit(registers)
    circ += stabilizer_list[0](registers, anQb=anQb_list[0], 
        syn_bit=syn_bit_list[0], reset=reset)
    circ += stabilizer_list[1](registers, anQb=anQb_list[1], 
        syn_bit=syn_bit_list[1], reset=reset)
    circ += stabilizer_list[2](registers, anQb=anQb_list[2], 
        syn_bit=syn_bit_list[2], reset=reset)
    circ += stabilizer_list[3](registers, anQb=anQb_list[3], 
        syn_bit=syn_bit_list[3], reset=reset)
...
if __name__ == "__main__":
    ...
    )['snapshots']['statevector']['testing'][0]

    log0 = logical[0][np.arange(128, step=4)]
    sv_test = sv_post_encoding[0:32]
    fid += state_fidelity(log0, sv_test)
    sv_post_encoding = results.data(
    )['snapshots']['statevector']['testing'][1]

    log0 = logical[0][np.arange(128, step=4)]
    sv_test = sv_post_encoding[0:32]
    fid += state_fidelity(log0, sv_test)
    sv_post_encoding = results.data(
    )['snapshots']['statevector']['testing'][2]

    log0 = logical[0][np.arange(128, step=4)]
    sv_test = sv_post_encoding[0:32]
    fid += state_fidelity(log0, sv_test)
    sv_post_encoding = results.data(
    )['snapshots']['statevector']['testing'][3]

    log0 = logical[0][np.arange(128, step=4)]
    sv_test = sv_post_encoding[0:32]
    fid += state_fidelity(log0, sv_test)
    sv_post_encoding = results.data(
    )['snapshots']['statevector']['testing'][4]

    log0 = logical[0][np.arange(128, step=4)]
    sv_test = sv_post_encoding[0:32]
    fid += state_fidelity(log0, sv_test)
    sv_post_encoding = results.data(
    )['snapshots']['statevector']['testing'][5]

    log0 = logical[0][np.arange(128, step=4)]
    sv_test = sv_post_encoding[0:32]
    fid += state_fidelity(log0, sv_test)
    sv_post_encoding = results.data(
    )['snapshots']['statevector']['testing'][6]

    log0 = logical[0][np.arange(128, step=4)]
    sv_test = sv_post_encoding[0:32]
    fid += state_fidelity(log0, sv_test)
    sv_post_encoding = results.data(
    )['snapshots']['statevector']['testing'][7]

    log0 = logical[0][np.arange(128, step=4)]
    sv_test = sv_post_encoding[0:32]
    fid += state_fidelity(log0, sv_test)
    sv_post_encoding = results.data(
    )['snapshots']['statevector']['testing'][8]

    log0 = logical[0][np.arange(128, step=4)]
    sv_test = sv_post_encoding[0:32]
    fid += state_fidelity(log0, sv_test)
    sv_post_encoding = results.data(
    )['snapshots']['statevector']['testing'][9]
