# https://github.com/Valerio-Amico/valerio_amico_quantum_00/blob/f509c202d2b1d7a7923a6348ddc095713a46c08d/version_08/funzioni01.py
...
def Trotter_step(type="our", N=10, time = np.pi):
    ...
    if type == "IBM":
        ...
        Trot_qr=QuantumRegister(3)
        Trot_qc=QuantumCircuit(Trot_qr,name="Trotter_step")
        Trot_qc.append(XX_qc,[0,0+1])
        Trot_qc.append(YY_qc,[0,0+1])
        Trot_qc.append(ZZ_qc,[0,0+1])
        Trot_qc.append(XX_qc,[1,1+1])
        Trot_qc.append(YY_qc,[1,1+1])
        Trot_qc.append(ZZ_qc,[1,1+1])
...
def add_check(ising, qr, cr_a, check_type, qubit_check, reset, m, order, i, steps):
    ...
    if check_type=="magnetization2":
        ...
        for _ in range(reset[2]):
            ising.reset(qr[qubit_check[2]])
        for _ in range(reset[3]):
            ising.reset(qr[qubit_check[3]])
        #ising.barrier()
        m+=4
