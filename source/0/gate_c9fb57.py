# https://github.com/wugaxp/z-quantum-core/blob/6ab6bf502817b71c0eaa7ca92dd8df1854609400/src/python/zquantum/core/circuit/_gate.py
...
class Gate(object):
    ...
    def to_unitary(self):
        ...
        """
        if self.name in {'X', 'Y', 'Z', 'H', 'S', 'T', 'PHASE', 'Rx', 'Ry', 'Rz',\
             'ZXZ', 'RH', 'CNOT', 'CZ', 'CPHASE', 'SWAP', 'XX', 'YY', 'ZZ'}: # gates defined in cirq
            return self.to_cirq().gate._unitary_()
        else:
            raise NotImplementedError('Gate {} currently not supported.'.format(self.name))
    ...
    def to_cirq(self, input_cirq_qubits=None):
        ...
        if self.name == 'ZXZ': # PhasedXPowGate gate 
            g = cirq.PhasedXPowGate(phase_exponent = params[0]/pi, exponent = params[1]/pi)
        ...
        if self.name == 'CPHASE':
            return cirq.CZPowGate(exponent=params[0]/pi)(cirq_qubits[0],cirq_qubits[1])
        if self.name == 'SWAP':
    ...
    def to_qiskit(self, qreg=None):
        ...
        if self.name == 'Rx':
            return [qiskit.extensions.standard.RXGate(params[0]), [qiskit_qubits[0]], []]
        if self.name == 'Ry':
            return [qiskit.extensions.standard.RYGate(params[0]), [qiskit_qubits[0]], []]
        if self.name == 'Rz' or self.name == 'PHASE':
            return [qiskit.extensions.standard.RZGate(params[0]), [qiskit_qubits[0]], []]
        if self.name == 'ZXZ': # PhasedXPowGate gate (from cirq)
            # Hard-coded decomposition is used for now.
            return [qiskit.extensions.standard.RXGate(-params[0]), [qiskit_qubits[0]], [],
                    qiskit.extensions.standard.RZGate( params[1]), [qiskit_qubits[0]], [],
                    qiskit.extensions.standard.RXGate( params[0]), [qiskit_qubits[0]], []]
        if self.name == 'RH': # HPowGate (from cirq)
            # Hard-coded decomposition is used for now.
            return [qiskit.extensions.standard.RYGate(pi/4),      [qiskit_qubits[0]], [],
                    qiskit.extensions.standard.RZGate(params[0]), [qiskit_qubits[0]], [],
                    qiskit.extensions.standard.RYGate(-pi/4),     [qiskit_qubits[0]], []]
        ...
        if self.name == 'XX':
            ...
            return [qiskit.extensions.standard.HGate(),            [qiskit_qubits[0]],                   [],
                    qiskit.extensions.standard.HGate(),            [qiskit_qubits[1]],                   [],
                    qiskit.extensions.standard.CnotGate(),         [qiskit_qubits[0], qiskit_qubits[1]], [],
                    qiskit.extensions.standard.RZGate(params[0]*2),[qiskit_qubits[1]],                   [],
                    qiskit.extensions.standard.CnotGate(),         [qiskit_qubits[0], qiskit_qubits[1]], [],
                    qiskit.extensions.standard.HGate(),            [qiskit_qubits[0]],                   [],
                    qiskit.extensions.standard.HGate(),            [qiskit_qubits[1]],                   []]
        if self.name == 'YY':
            # Hard-coded decomposition is used for now. The compilation is inspired by the approach described in arXiv:1001.3855 [quant-ph]
            return [qiskit.extensions.standard.SGate(),            [qiskit_qubits[0]],                   [],
                    qiskit.extensions.standard.SGate(),            [qiskit_qubits[1]],                   [],
                    qiskit.extensions.standard.HGate(),            [qiskit_qubits[0]],                   [],
                    qiskit.extensions.standard.HGate(),            [qiskit_qubits[1]],                   [],
                    qiskit.extensions.standard.CnotGate(),         [qiskit_qubits[0], qiskit_qubits[1]], [],
                    qiskit.extensions.standard.RZGate(params[0]*2),[qiskit_qubits[1]],                   [],
                    ...
                    qiskit.extensions.standard.HGate(),            [qiskit_qubits[0]],                   [],
                    qiskit.extensions.standard.HGate(),            [qiskit_qubits[1]],                   [],
                    qiskit.extensions.standard.SdgGate(),          [qiskit_qubits[0]],                   [],
                    qiskit.extensions.standard.SdgGate(),          [qiskit_qubits[1]],                   []]
        if self.name == 'ZZ':
            # Hard-coded decomposition is used for now. The compilation is inspired by the approach described in arXiv:1001.3855 [quant-ph]
            return [qiskit.extensions.standard.CnotGate(),         [qiskit_qubits[0], qiskit_qubits[1]], [],
                    qiskit.extensions.standard.RZGate(params[0]*2),[qiskit_qubits[1]],                   [],
        ...
        if self.name == 'BARRIER':
            return [qiskit.extensions.standard.Barrier(len(qiskit_qubits)),         qiskit_qubits,                    []]
    ...
    def to_qpic(self):
        ...
        # Generic gate
            ...
            if len(self.params) > 0:
                qpic_string += ' width={}'.format(6*len(self.name + self.get_param_string()))
    ...
    def from_cirq(cls, cirq_gate, Qubit_list):
        ...
        if len(gate_string.split('(')) > 1: # test if the gate string contains parentheses
            parsed_gatename = gate_string.split('(')[0]
            if parsed_gatename == 'PhasedX' or parsed_gatename == 'PhX':
                output.name = 'ZXZ'
                output.params = [cirq_gate.gate.phase_exponent*pi, cirq_gate.gate.exponent*pi]
                ...
                raise NotImplementedError('The cirq gate {} is currently not supported'.format(parsed_gatename))
        else: 
            parsed_gatename = str(cirq_gate.gate).replace('**',' ').replace('^',' ').split()
            name_str = parsed_gatename[0]
            if name_str in {'X', 'Y', 'Z', 'H', 'T', 'S', 'CNOT', 'SWAP',\
                'CZ', 'XX', 'YY', 'ZZ'}:
                if len(parsed_gatename) == 1: # discrete gate
                    ...
                    if name_str == 'X':
                        ...
                        raise NotImplementedError('The cirq gate {} is currently not supported in exponential format'.format(name_str))
            else:
                raise NotImplementedError('The cirq gate {} is currently not supported'.format(name_str))
