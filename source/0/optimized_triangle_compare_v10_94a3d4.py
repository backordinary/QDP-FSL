# https://github.com/revilooliver/Quantum-Hackathon-2019/blob/102c97500bf442070c4709337b0a892738845583/final/optimized_triangle_compare_v10.py
...
# Perform inversion about the average amplitude.
inversion_about_average(circ,q,ancilla_bits)
circ.measure(q[2*0],c[2*0])
circ.measure(q[(2*0)+1],c[2*0 + 1])
circ.measure(q[2*1],c[2*1])
circ.measure(q[(2*1)+1],c[2*1 + 1])
circ.measure(q[2*2],c[2*2])
circ.measure(q[(2*2)+1],c[2*2 + 1])
circ.measure(q[2*3],c[2*3])
circ.measure(q[(2*3)+1],c[2*3 + 1])
circ.measure(q[2*4],c[2*4])
circ.measure(q[(2*4)+1],c[2*4 + 1])
circ.measure(q[2*5],c[2*5])
circ.measure(q[(2*5)+1],c[2*5 + 1])
circ.measure(q[2*6],c[2*6])
circ.measure(q[(2*6)+1],c[2*6 + 1])
