# https://github.com/dberga/quantum-experiments/blob/4f13e993fa4382496b7be5caf35dd4898b8d271e/qiskit_tutorials/05_connector_cifar100_first2classes_hybridqnn.py
# Necessary imports
...
while n_samples_show > 0:
    ...
    axes[n_samples_show - 1].imshow(np.moveaxis(images[0].numpy().squeeze(),0,-1))
...
plt.savefig("plots/05_classification_altclass_subplots_labeled.png")
...
# Use pre-defined torchvision function to load test data
...
n_channels = X_train.data.shape[3] # rgb or grayscale
...
plt.savefig("plots/05_classification_altclass_hybridqnn.png")
...
with no_grad():
    for batch_idx, (data, target) in enumerate(test_loader):
        ...
        pred = output.argmax(dim=1, keepdim=True)
        axes[count].imshow(np.moveaxis(data.numpy().squeeze(),0,-1))
        pred_class = classes_list[pred.item()]
        axes[count].set_xticks([])
        axes[count].set_yticks([])
        axes[count].set_title(pred_class)
        count += 1

plt.savefig("plots/05_classification_altclass_subplots_pred.png")
