# https://github.com/matteoacrossi/adapt_ic-povm/blob/9023ae3f20caab2b3cab0a900617fc03ba136b89/tomography_script.py
import pandas as pd
import numpy as np
...
if __name__ == "__main__":
    ...
    avg_fidelities_k = {}
    print(2)
    avg_fidelities_k[2] = sample_df.groupby(
        ["qubits", "method", "id"]
    ).parallel_apply(lambda x: kwise_fidelity(x, 2))
    print(3)
    avg_fidelities_k[3] = sample_df.groupby(
        ["qubits", "method", "id"]
    ).parallel_apply(lambda x: kwise_fidelity(x, 3))
    print(4)
    avg_fidelities_k[4] = sample_df.groupby(
        ["qubits", "method", "id"]
    ).parallel_apply(lambda x: kwise_fidelity(x, 4))
    print(5)
    avg_fidelities_k[5] = sample_df.groupby(
        ["qubits", "method", "id"]
    ).parallel_apply(lambda x: kwise_fidelity(x, 5))
